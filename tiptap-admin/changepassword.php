...
{			
	...
	if($npassword==$cpassword){		
		$login_query = "SELECT * from tiptap_adminuser where id=$loginid";
		...
		foreach($menbanner_res2 as $login_res) {								
			...
			{				
				$login_query2 = "UPDATE tiptap_adminuser SET password='$cpassword' where id=$loginid";
				$database = new Database();
				$dbCon = $database->getConnection();
	...
	{			
		...
		if (($ext == "jpg") || ($ext == "jpeg") || ($ext == "png")) {			
			...
			if (move_uploaded_file($_FILES['image1']['tmp_name'], $target_file1)) {					
				$login_query = "UPDATE tiptap_adminuser SET profilepic='$blocation1' WHERE id=$loginid";
				$database = new Database();
				$dbCon = $database->getConnection();
				$stmt = $dbCon->prepare($login_query);  
				$stmt->execute();
