...
{	
	...
	$login_query = "INSERT INTO tiptap_adminsubmenu (adminmenuid, submenuname, link) VALUES ('$menu', '$subname', '$link')";
	...
	$banner_que = "SELECT * from tiptap_adminuser where 1=1";
	...
	foreach($menbanner_res as $banner_result)
	{
		$id = $banner_result['id'];
		if($id==$loginid){			
			$product_sql = "INSERT INTO `tiptap_usermenulist` (`adminmenuid`, `menusubmenurel`, `adminsubmenuid`, `userid`, `status`) VALUES (0, $menu, $adsmid, $id, 1)";
					  ...
					  <div class="form-group">
						...
						<div class="col-md-3 col-sm-3 col-xs-12">
							<select name="menu" id="menu" required class="form-control" >
								...
								<?php
								$j = 6;					
								$banner_que = "SELECT * from tiptap_adminmenu where 1=1 ORDER BY id ASC";
								$database1 = new Database();
								$dbCon1 = $database1->getConnection();
								$stmt1 = $dbCon1->prepare($banner_que);  
								$stmt1->execute();	
